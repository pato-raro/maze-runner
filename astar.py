        ...
        for idx, step in list(enumerate(path)):
            ...
            if step[0] < path[idx+1][0]:
                ...
                convertedPath.append(
                    ["down", (stop_time - self.start_time)*1000])
            if step[0] > path[idx+1][0]:
                stop_time = timeit.default_timer()
                convertedPath.append(
                    ["up", (stop_time - self.start_time)*1000])
            # [3, 2] vs [3,3]
            if step[1] < path[idx+1][1]:
                stop_time = timeit.default_timer()
                convertedPath.append(
                    ["right", (stop_time - self.start_time)*1000])
            if step[1] > path[idx+1][1]:
                stop_time = timeit.default_timer()
                convertedPath.append(
                    ["left", (stop_time - self.start_time)*1000])
        return convertedPath

    def write_to_text(self, content, filename='action.txt'):
        filename = 'actions/' + self.name + '.txt'
        f = open(filename, "w")
        for item in content:
            f.write(item[0])
            f.write(' ')
